...
#   [[タイトル]] / [[タイトル|表示名]]
#   [[slug]]（ソースと同じディレクトリ。表示はタイトル、| で上書き可）
#   [[full_path/slug]]（ルートからのディレクトリ full_path + スラッグ。先頭 / は任意）
...
class MemoWikiLinks
  ...
  def resolve(target)
    ...
    if source_directory_id
      slug_resolved = resolve_slug_in_directory(source_directory_id, key.downcase)
      return slug_resolved if slug_resolved
    end
  ...
  def resolve_path_slug(target)
    dir_part, _, slug_part = target.rpartition("/")
  ...
  def load_index!
    ...
    @scope.pluck(:id, :title, :memo_directory_id, :slug).each do |id, title, dir_id, slug|
      ...
      if slug.present?
        @slug_by_directory[[dir_id, slug.downcase]] = id
      end
    end
