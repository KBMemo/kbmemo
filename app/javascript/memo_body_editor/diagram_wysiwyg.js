...
function svgIntrinsicSize(svg) {
  ...
  let width =
    widthAttr && !widthAttr.includes("%") ? parseSvgLength(widthAttr) : null
  let height =
    heightAttr && !heightAttr.includes("%") ? parseSvgLength(heightAttr) : null
...
class DiagramPreviewWidget extends WidgetType {
  ...
  toDOM() {
    ...
    fig.className = this.block
      ? "cm-wysiwyg-diagram cm-wysiwyg-diagram--block"
      : "cm-wysiwyg-diagram cm-wysiwyg-diagram--inline"
...
/**
 * Phase 5e CM: `diagram::` → キャッシュ済み SVG（object）プレビュー。
 */
